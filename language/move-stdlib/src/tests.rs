...
fn check_that_docs_are_updated() {
    ...
    crate::doc::build_stdlib_doc(&temp_dir.path().to_string_lossy());

    let res = check_dirs_not_diff(&temp_dir, crate::doc::move_stdlib_docs_full_path());
    assert!(
        res.is_ok(),
...
fn check_that_the_errmap_is_updated() {
    ...
    crate::doc::build_error_code_map(&temp_file.path().to_string_lossy());

    assert!(
        file_diff::diff(
            &temp_file.path().to_string_lossy(),
            &crate::doc::move_stdlib_errmap_full_path()
        ),
